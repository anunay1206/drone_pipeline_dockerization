...
def update_project(
    ...
    """Update project metadata and pipeline parameters.

    Heavy derived artifacts are regenerated by the next analyze/finalize call.
    Parameter/model changes invalidate any submitted labels because they may no
    longer match the next clustering output.
    """
    if project.state in {"ANALYZING", "FINALIZING"}:
        raise HTTPException(409, f"Cannot update project while state is {project.state}")
    ...
    if body.model_key is not None:
        try:
            model_key, _ = resolve_model_path(body.model_key)
        except ValueError as e:
            raise HTTPException(400, str(e))
        if model_key != project.model_key:
            project.model_key = model_key
            changed_pipeline = True

    if body.source_epsg is not None:
        project.source_epsg = body.source_epsg

    if body.params is not None:
        updates = body.params.model_dump(exclude_none=True)
        if updates:
    ...
    if changed_pipeline:
        ...
        project.error = None
        project.state = "UPLOADED" if project.orthos else "CREATED"
...
def upload_ortho(
    ...
    if project.state in {"ANALYZING", "FINALIZING"}:
        raise HTTPException(
            409, f"Cannot replace orthomosaic while state is {project.state}"
        )
